...
class TaskService {
    ...
    async createTask(taskData) {
        ...
        if (!validation.valid) throw new Error(`ข้อมูลไม่ถูกต้อง: ${validation.errors.join(', ')}`);
        ...
        if (createdTask.priority === 'HIGH') console.log(`🔥 สร้างงานลำดับความสำคัญสูง: ${createdTask.title}`);
        return createdTask;
    }
    ...
    async deleteTask(id) {
        ...
        if (task.priority === 'HIGH') console.log(`⚠️ กำลังลบงานลำดับความสำคัญสูง: ${task.title}`);
    ...
    async getStatistics() {
        ...
        return {
            ...
            byStatus: { TODO: counts.TODO || 0, IN_PROGRESS: counts.IN_PROGRESS || 0, DONE: counts.DONE || 0 },
            byPriority: {
    ...
    async moveToNextStatus(id) {
        ...
        if (nextStatus === task.status) throw new Error('งานนี้เสร็จสมบูรณ์แล้ว');
